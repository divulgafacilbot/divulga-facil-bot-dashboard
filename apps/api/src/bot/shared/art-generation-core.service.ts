...
import { BrandConfig, ArtFormat } from '../../services/image-generation/art-generator.service.js';
import { LayoutPreferences } from '../../services/layout-preferences.service.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

/**
 * Art Generation Core Service
 * Shared art generation utilities for all bots (Arts Bot, Pinterest Bot)
 */
export class ArtGenerationCoreService {
  /**
  ...
  /**
   * Build legend text for feed format
   */
  buildLegendText(
    product: ProductData,
  ...
  /**
   * Get story fields for rendering
   */
  getStoryFields(
    product: ProductData,
    brandConfig: BrandConfig,
  ...
  /**
   * Normalize price display (swap price/originalPrice if inverted)
   */
  normalizePriceDisplay(product: ProductData): {
    price: number | null;
  ...
  /**
   * Wrap text into multiple lines based on max characters
   */
  ...
  async generateFeedArt(
    ...
    // Composite: template + product image
 ...
 */
export const artGenerationCoreService = new ArtGenerationCoreService();
